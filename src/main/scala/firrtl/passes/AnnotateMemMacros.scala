...
object AnalysisUtils {
  ...
  def getConnectOrigin(connects: Connects)(node: Expression): Expression = {
    connects get node.serialize match {
  ...
  private def getOrigin(connects: Connects, e: Expression): Expression = e match { 
    ...
    case _: WRef | _: SubField | _: SubIndex if connects contains e.serialize =>
      getConnectOrigin(connects)(e)
    case _ => e
  }
...
object AnnotateMemMacros extends Pass {
  ...
  def getMaskBits(connects: Connects, wen: Expression, wmask: Expression): Option[Int] = {
    ...
    val wmaskOrigins = create_exps(wmask) map getConnectOrigin(connects)
    // all wmask bits are equal to wmode/wen or all wmask bits = 1(for redundancy checking)
    val redundantMask = wmaskOrigins forall (x => weq(x, wenOrigin) || weq(x, one))
    if (redundantMask) None else Some(wmaskOrigins.size)
  }
